      ...
      var $dom = $(this);
        ...
        if (options !== undefined && (typeof(options) == "boolean" || typeof(options) == "string")) { // Force flip the DOM
          ...
          var rotateAxis = "rotate" + (settings.axis.toLowerCase() == "x" ? "x" : "y"), 
              perspective = jQuery(window).width();

          $dom.find($dom.data("back")).css({
            transform: rotateAxis + "(" + (settings.reverse? "180deg" : "-180deg") + ")"
          });
		  var explore_width=jQuery(window).width()/2;
		  card_offset=$dom.parent().parent().offset();
		  new_top=-(card_offset.top);
		  new_left=-(card_offset.left);

          $dom.css({
            perspective: perspective,
            position: "relative",
            top: new_top+jQuery(window).scrollTop()+(jQuery(window).height()/2),
            left: new_left+(explore_width)
          });
